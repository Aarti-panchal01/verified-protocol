...
@pytest.fixture()
def verified_protocol_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> VerifiedProtocolClient:
    factory = algorand_client.client.get_typed_app_factory(
        VerifiedProtocolFactory, default_sender=deployer.address
    )
...
def test_says_hello(verified_protocol_client: VerifiedProtocolClient) -> None:
    result = verified_protocol_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    verified_protocol_client: VerifiedProtocolClient,
) -> None:
    result = (
        verified_protocol_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
